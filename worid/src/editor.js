    ...
    update(input) {
      if (input.justPressed('KeyE')) {
        if (!active) {
          ...
          // Check for floating letter pickup
          const touched = findTouchedFloating(scene, state.player);
          if (touched !== -1) {
            const letter = scene.letters[touched];
            if (state.addToInventory(letter.char)) {
              scene.letters = scene.letters.filter(l => l !== letter);
              scene.words = scene.words.filter(w => w.text.length !== 1 || !w.floating || w.text !== letter.char);
            }
            return;
          }
        } else {
